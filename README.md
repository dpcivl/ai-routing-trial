...
| `coding` | 코드 작성, 디버깅, 리팩터링 | `qwen2.5-coder:7b` (회사 vLLM에서는 `devstral-small-2`) |
| `vision` | 이미지 분석, UI/UX 디자인 질문 | `gemma4:12b` |

라우터용 소형 LLM 분류기는 `qwen3.5:2b`를 씁니다 (`--router llm`).
...
# 3. 라우터 정확도 평가 + 보고서 생성 (규칙 기반은 Ollama 없이 동작)
...
uv run ai-router ask "git 커밋 메시지 수정법" --router llm      # 소형 LLM 분류기로 라우팅
uv run ai-router eval-routing --router llm                        # LLM 라우터 정확도 평가
...
src/ai_router/
  ...
  routers/               라우터 구현 (rule, llm. embedding은 보류)
...
- [ ] ② 임베딩 유사도 라우터 (보류)
- [x] ③ 소형 LLM 분류기 라우터 (구현 완료, 가짜 백엔드로 테스트. **실제 모델 정확도는 아직 미측정**)
- [ ] 라우터 3종 비교 보고서
